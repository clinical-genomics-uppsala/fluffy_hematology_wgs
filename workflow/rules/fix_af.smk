rule fix_af:
    ...
    output:
        vcf=temp(
            "parabricks/pbrun_mutectcaller_t/{sample}_{type}.normalized.vep.filter.germline.fix_af.vcf"
        ),
    log:
    ...
    resources:
        mem_mb=config.get("fix_af", {}).get(
            "mem_mb", config["default_resources"]["mem_mb"]
        ),
        mem_per_cpu=config.get("fix_af", {}).get(
            "mem_per_cpu", config["default_resources"]["mem_per_cpu"]
        ),
        partition=config.get("fix_af", {}).get(
            "partition", config["default_resources"]["partition"]
        ),
        threads=config.get("fix_af", {}).get(
